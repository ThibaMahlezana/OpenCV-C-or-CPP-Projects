...
// OpenCV Includes
#include <opencv2/core/utilities.hpp>
...
shared_ptr<MultipleImageWindow> wiw;
...
void ConnectedComponents(Mat img){
    ...
    if(num_objects < 2){
        cout << "[INFO] No objects detected" << endl;
        return;
    }
    else{
        cout << "[INFO] Number of objects detected: " << num_objects - 1 << endl;
    }

    // Creating output image coloring the objects
...
void FindContoursBasic(Mat img){
    ...
    if(contours.size() == 0){
        ...
        cout << "[INFO] Number of objects detected: " << contours.size() << endl;
    }
    RNG rng(0xFFFFFFFF);
...
int main(int argc, char** argv){
	...
	parser.about("Object Detector v1.0.0");
