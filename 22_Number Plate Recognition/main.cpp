...
int main(int argc, char** argv){
    ...
    // Checking if the user specify image to process
    if(argc >= 2){
        filename = argv[1];
    ...
    cout << "[INFO] working with file: " << filename_whithoutExt << "\n";
    ...
    // For each possible plate, classify with svm classify if it's a plate or not
    vector<Plate> plates;
    ...
    cout << "[INFO] Number of plates detected: " << plates.size() << endl;
